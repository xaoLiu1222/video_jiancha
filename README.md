...
**重要说明**：审核后的视频会自动分类到**输入目录下**的子文件夹中：
- `/media/usb/videos/approved/` - 通过的视频
- `/media/usb/videos/rejected/` - 拒绝的视频
- `/media/usb/videos/manual_review/` - 需人工复核
- `/media/usb/videos/review_results.json` - 审核报告
...
审核完成后，视频会自动移动到**输入目录下**的对应子目录：

| 结果 | 目录 | 说明 |
|------|------|------|
| ✅ 通过 | `<输入目录>/approved/` | 符合所有标准 |
| ❌ 拒绝 | `<输入目录>/rejected/` | 违反审核规则 |
| ❓ 复核 | `<输入目录>/manual_review/` | 需人工确认 |

详细报告保存在 `<输入目录>/review_results.json`

**示例**：如果审核 `/media/usb/videos/`，结果会保存在：
